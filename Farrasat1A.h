...
public:
	...
	void CalTimeSignal(int myRank,int commSize);



	// Solve FW-H equations using frequency-domain formulation
	void CalFreqSpectra(int myRank,int commSize);

	// Solve 2D FW-H  frequency-domain formulation
	void CalFreqSpectra2D(int myRank,int commSize);
	...
	double GetPureFWHKernelTime(void) const {
    return pureFWHKernelTime_s;
	}

	double GetInternalMPITime(void) const {
		return internalMPITime_s;
	}
...
private:
	...
	double pureFWHKernelTime_s = 0.0;   // 纯 FW-H 积分 kernel 时间（秒）
	double internalMPITime_s   = 0.0;   // CalTimeSignal 内部 MPI_Barrier 时间（秒）
